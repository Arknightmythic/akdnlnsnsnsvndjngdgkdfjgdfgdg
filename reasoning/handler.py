...
class ReasoningHandler:
    ...
    def run_reasoning(self, file_id: str):
        ...
        with self.reasoning_service.engine.begin() as conn:
            conn.execute(text("UPDATE uploaded_files SET reasoning_status = 'PROCESSING' WHERE id = :fid"), {"fid": file_id})
        ...
        with self.reasoning_service.engine.begin() as conn:
            conn.execute(text("UPDATE uploaded_files SET reasoning_status = :status WHERE id = :fid"), {"status": status, "fid": file_id})
